...
可以做出靜態的網頁，甚至連表單都能用純 HTML（`<form action method>`）把資料送到伺服器！但如果你想要更豐富的互動效果（像是點擊按鈕即時更新畫面內容、表單送出後不整頁刷新就顯示結果），就需要再學 JavaScript。你可以把 JavaScript 想成 Word 裡的「巨集（Macro）」— 自動化的腳本。
