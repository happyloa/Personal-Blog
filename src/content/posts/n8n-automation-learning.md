...
我之前有聽過 Zapier、Make 這類自動化工具，但它們大多是付費的，免費額度也很有限。後來看到很多人在推薦 n8n，說它原始碼公開、可以自己架在自己的伺服器上，免費又彈性，剛好很適合拿來串接 Facebook 的訊息和後續的處理流程，就決定從它開始研究。
...
```mermaid
flowchart LR
    A["顧客在粉專<br>發訊息"] --> B["Webhook<br>觸發流程"]
    B --> C["AI Agent<br>節點"]
    C --> D["回覆顧客"]
    C -. 需要時自行調用 .-> T1["查詢最新方案"]
    C -. 需要時自行調用 .-> T2["查詢分店地址"]
